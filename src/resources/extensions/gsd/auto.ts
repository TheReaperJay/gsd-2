...
async function dispatchNextUnit(
  ...
  // Write lock with Pi session file — all providers dispatch through Pi's agent loop
  writeLock(lockBase(), unitType, unitId, completedUnits.length, ctx.sessionManager.getSessionFile());
  ...
  // Update per-dispatch state for the stream adapter before injecting the prompt.
  // The stream adapter's getter callbacks read these values during dispatch.
  setStreamAdapterUnitInfo(unitType, unitId);
  setStreamAdapterBasePath(lockBase());
  setStreamAdapterIsUnitDone(() => verifyExpectedArtifact(unitType, unitId, basePath));
