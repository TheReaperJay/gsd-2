...
// ─── claude-code credential type ─────────────────────────────────────────────

describe("AuthStorage — claude-code credential type", () => {
	it("set() accepts { type: 'claude-code' } without throwing", () => {
		const storage = inMemory({});
		assert.doesNotThrow(() => {
	...
	it("get() returns the credential after set()", () => {
		const storage = inMemory({});
		storage.set("claude-code", { type: "claude-code" } as import("./auth-storage.js").ClaudeCodeCredential);
		const cred = storage.get("claude-code");
		assert.ok(cred !== undefined, "credential should exist");
		assert.equal(cred!.type, "claude-code");
	});

	it("has() returns true after set()", () => {
		const storage = inMemory({});
		storage.set("claude-code", { type: "claude-code" } as import("./auth-storage.js").ClaudeCodeCredential);
		assert.equal(storage.has("claude-code"), true);
	});

	it("hasAuth() returns true after set()", () => {
		const storage = inMemory({});
		storage.set("claude-code", { type: "claude-code" } as import("./auth-storage.js").ClaudeCodeCredential);
	...
	it("list() includes 'claude-code' after set()", () => {
		const storage = inMemory({});
		storage.set("claude-code", { type: "claude-code" } as import("./auth-storage.js").ClaudeCodeCredential);
		assert.ok(storage.list().includes("claude-code"), "list() should include 'claude-code'");
	});

	it("inMemory() initialised with claude-code credential is retrievable via get()", () => {
		const storage = inMemory({ "claude-code": { type: "claude-code" } });
		const cred = storage.get("claude-code");
